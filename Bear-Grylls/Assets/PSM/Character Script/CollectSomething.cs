...
namespace PSM
{
    ...
    public class CollectSomething : InterplayAction
    {
        ...
        protected override void InterplaySuccessCode()
        {
            ...
            if (selector.SelectedObject != null)
            {
                collectableObject = selector.SelectedObject.GetComponent<CollectableObject>();
                HavingCollectableActionComponent havingCollectableActionComponent = selector.SelectedObject.GetComponent<HavingCollectableActionComponent>();
                if (havingCollectableActionComponent != null) havingCollectableActionComponent.ActionCollected();
            }

            if (collectableObject != null)
            {
                ItemInfoDroped[] itemInfoDroped = collectableObject.DropItem();
                for(int i = 0; i < itemInfoDroped.Length; ++i)
                {
                    Item item = (Item)Activator.CreateInstance(itemInfoDroped[i].ItemType);

                    inventory.AddItem(item, itemInfoDroped[i].ItemNumber);
                }
            }
